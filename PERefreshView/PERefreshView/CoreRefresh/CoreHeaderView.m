...
- (void)adjustStateWithContentOffset
{
    ...
    if (self.scrollView.isDragging) {
        ...
        if (self.state == CoreHeaderViewRefreshStateNorMal && currentOffsetY >= happenOffsetY) {
            ...
            // 转为即将刷新状态
            self.state = CoreHeaderViewRefreshStateReleaseToRefreshing;
            
        } else if (self.state == CoreHeaderViewRefreshStateReleaseToRefreshing && currentOffsetY < happenOffsetY) {
            
            // 转为普通状态
            self.state = CoreHeaderViewRefreshStateNorMal;
        }
    } else if (self.state == CoreHeaderViewRefreshStateReleaseToRefreshing) {// 即将刷新 && 手松开
...
{
    CGFloat nowY = currentOffsetY-deltaValue;
    self.showView.progress = .00001f;
    if(nowY <= 0) return;
    
    //计算进度
    CGFloat progress = nowY/(happenOffsetY-deltaValue);
...
-(void)setState:(CoreHeaderViewRefreshState)state
{
    ...
    switch (state) {
        ...
        case CoreHeaderViewRefreshStateReleaseToRefreshing://普通状态->松手立即刷新
        ...
        case CoreHeaderViewRefreshStateRefreshing://松手立即刷新->刷新中,特别的在业务中直接从普通状态->刷新中
            if(oldState==CoreHeaderViewRefreshStateReleaseToRefreshing || oldState==CoreHeaderViewRefreshStateNorMal) [self stateRefreshing];
            break;
            
        case CoreHeaderViewRefreshStateFailed://刷新中->刷新失败
            if(oldState==CoreHeaderViewRefreshStateRefreshing) [self stateRefreshingFailed];
            break;
            
        case CoreHeaderViewRefreshStateNoMoreData://刷新中->刷新成功，无更多数据
            if(oldState==CoreHeaderViewRefreshStateRefreshing) [self stateSuccessedResultNoMoreData];
            break;
            
        case CoreHeaderViewRefreshStateSuccessed://刷新中->刷新成功，数据展示中
...
-(void)stateRefreshingFailed
{
    ...
    //添加动画
//    [self.showView.layer addAnimation:[CAAnimation shakeAnim] forKey:@"shakeAnim"];
...
- (void)beginRefreshing
{
    if (self.state == CoreHeaderViewRefreshStateRefreshing) {
        ...
        if (self.window) {
            ...
            _state = CoreHeaderViewRefreshStateReleaseToRefreshing;

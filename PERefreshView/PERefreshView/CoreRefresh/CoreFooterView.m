...
+(instancetype)footer{
    return [[CoreFooterView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, CoreRefreshFooterViewH)];
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self configINterfaceWithShowAIView:YES text:@"数据正在加载"];
    }
    return self;
}
...
-(void)setState:(CoreFooterViewRefreshState)state{
    ...
    switch (state) {
        ...
        case CoreFooterViewRefreshStateNoMoreData://刷新正常结束：无更多数据
            [self stateSuccessedResultNoMoreData];
            break;
            
        case CoreFooterViewRefreshStateSuccess://刷新正常结束，数据正在展示中
...
#pragma mark 刷新失败
-(void)stateFailed{
    [self configINterfaceWithShowAIView:NO text:@"加载失败，点击重试"];
}

#pragma mark 刷新成功->无更多数据:数据量小于pagesize的都进入这个状态
-(void)stateSuccessedResultNoMoreData{
    [self configINterfaceWithShowAIView:NO text:@"没有更多数据"];
}

#pragma mark 数据正在展示中
-(void)stateSuccessedResultDataShowing{
    [self configINterfaceWithShowAIView:NO text:@"数据正在展示中"];
...
-(void)configINterfaceWithShowAIView:(BOOL)showAIView text:(NSString *)text
{
    ...
    self.messageLabel.center = CGPointMake(self.ex_width/2 + (showAIView ? 10 : 0), self.ex_height/2);
    self.aiView.center = CGPointMake(_messageLabel.ex_x - 30, self.ex_height/2);
}
...
- (void)adjustStateWithContentOffset
{
    ...
    //此代码放在这里性能更好，避免无效计算
    if(self.state==CoreFooterViewRefreshStateNoMoreData) return;
